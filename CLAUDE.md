...
**23 Sep session:** grab bug fixed (`cb21ea0`, piece jumped to ray∩floor behind it) · sun leaks sealed + floor "staying" label removed (`4d9ae6f`) · presets laid out as living rooms, 12–14 pieces, layout test; `FitCollider` rotated-box bug fixed (`5d18ebe`) · simulator ray converges on gaze, menu lift actually applied via `anchoredPosition3D` (`2783758`). Kaike confirmed: **`G` grab works, rooms better.** Still reported: can't aim/select, menu hard to reach (menu rides the left hand which rides the head → centre-screen ray can never reach it).

**Decision 23 Sep: no headset for IP2a — test in the editor with mouse.** WIP commit (untested, scene NOT rebuilt): `EditorDesktopRig` on the XR Origin — free cursor aims the right ray (`EditorGazeAim` reads `TryCursorRay`), hold right mouse + drag = look (camera TrackedPoseDriver off), WASD walk clamped in room, clicks/G still via simulator, F4 = raw simulator; `SimulatorCursorLock` stands down while it's on.

**Tomorrow, in order:** (1) Unity must be **closed** (it was wedged: I recompiled during Play → XRI `TrackedDeviceGraphicRaycaster` static dict lost → `KeyNotFoundException` on ControllerMenu; log hit 13 GB, truncated). (2) Headless: `SceneBuilder.Build`, EditMode + PlayMode (expect new `EditorDesktopRigTests` + `DesktopMode_OnInEditor_…`). (3) Open editor, Play, verify cursor-aim + menu click + right-drag look + WASD. Unverified risks: does the classic simulator still send left click → trigger with the cursor unlocked; does XRUIInputModule's mouse input double-fire on the menu canvas (if so, `enableMouseInput = false` in desktop mode). (4) Then Kaike's lap. **Rule: check `isPlaying` and `editor_stop` before any Refresh/Build via the CLI.**
