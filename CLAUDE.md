...
## Current state — 11 Sep 2026, Week 7 (simulator feel pass started)

**Done:** IP1 tested Fri 28 Aug (n=5, simulator; hold-up loop entered 0/5) · Evaluation 1 submitted Wed 2 Sep · IP2a build T0–T14 green (3 Sep) · **11 Sep: simulator feel-pass tooling** — `EditorGazeAim` (editor-only: right controller rides the head, plain mouse = look, left click = trigger, `G` = grip, scroll = twist, `F4` = raw simulator; forces the classic XR Device Simulator to target HMD + right controller because it only feeds buttons to a targeted controller) · menu closes on **right B** (device) and `Backspace` (editor) as well as left Y; `SchemeCycler` yields to an open menu. EditMode 138, PlayMode 22. Plan + STATUS: `docs/superpowers/plans/2026-09-03-ip2a-build.md`.

**Live-editor workflow (11 Sep):** `unity open <proj>` then `unity command {recompile,recompile_status,run_tests --mode EditMode,run_tests --mode PlayMode --async_tests + test_status,eval,editor_play,editor_stop,console}`. `SceneBuilder.Build()` via `eval`. Nothing new needs the headless chain while the editor is open. Do not run a batchmode editor against the project while the GUI editor has it open.
...
**Kaike next:** continue the simulator feel pass (mouse-driven now); report what feels wrong, in the order met · then the real controller in the simulator (`F4`) · voice pass on `ip2a/2026-09-25-ip2a-testing-plan.md` + `data-collection-sheet.md` · re-dress presets if wanted (`Assets/Editor/PresetAuthoring.cs`, then `Renovation → Build Presets`, then `SceneBuilder.Build`) · Quest `unauthorized` track (`ip2a/quest-sideload.md`) — **no studio entry since 28 Aug; hello-world on device is overdue (Wk 7 goal)**. **APK: build only when Kaike says go, right before a headset session.**
